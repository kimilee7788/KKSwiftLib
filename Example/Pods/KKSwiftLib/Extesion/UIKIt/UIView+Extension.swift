...
public extension JKPOP where Base: UIView {
    ...
    func drawCircle(fillColor: UIColor, strokeColor: UIColor, strokeWidth: CGFloat) {
        let ciecleRadius = self.base.km.width > self.base.km.height ? self.base.km.height : self.base.km.width
...
public extension JKPOP where Base: UIView {
    ...
    func addWater(markText: String, textColor: UIColor = UIColor.black, font: UIFont = UIFont.systemFont(ofSize: 12)) {
        let waterMark: NSString = markText.km.toNSString
        let textSize: CGSize = waterMark.size(withAttributes: [NSAttributedString.Key.font : font])
        // 多少行
        let line: NSInteger = NSInteger(self.base.km.height * 3.5 / 80)
        // 多少列：自己的宽度/(每个水印的宽度+间隔)
        let row: NSInteger = NSInteger(self.base.km.width / markText.km.rectWidth(font: font, size: CGSize(width: self.base.km.width, height: CGFloat(MAXFLOAT))))
        for i in 0..<line {
            for j in 0..<row {
    ...
    func toImage() -> UIImage? {
        ...
        UIGraphicsBeginImageContextWithOptions(self.base.frame.size, false, 0.0)
...
public extension JKPOP where Base: UIView {
    ...
    func getViewLayer(borderWidth: CGFloat = 0.5, borderColor: UIColor = .randomColor, backgroundColor: UIColor = .randomColor) {
        ...
        for subview in subviews {
            ...
            subview.km.getViewLayer(borderWidth: borderWidth, borderColor: borderColor, backgroundColor: backgroundColor)
        }
        #endif
    }
    ...
    func findSubview(type: UIResponder.Type, resursion: Bool)-> UIView? {
        ...
        for e in self.base.subviews.enumerated() {
            let tmpView = e.element.km.findSubview(type: type, resursion: resursion)
            if tmpView != nil {
                return tmpView
            }
        }
...
public extension JKPOP where Base: UIView {
    ...
    func addActionClosure(_ actionClosure: @escaping ViewClosure) {
        if let sender = self.base as? UIButton {
            sender.km.setHandleClick(controlEvents: .touchUpInside) { (control) in
            ...
            sender.km.addActionHandler({ (control) in
                actionClosure(nil, control, control.tag)
            }, for: .valueChanged)
        } else {
            _ = self.base.km.addGestureTap { (reco) in
...
public extension JKPOP where Base : UIView {
    ...
    func gradientColor(_ direction: JKViewGradientDirection = .horizontal, _ gradientColors: [Any], _ gradientLocations: [NSNumber]? = nil) {
        // 获取渐变对象
        let gradientLayer = CAGradientLayer().km.gradientLayer(direction, gradientColors, gradientLocations)
        // 设置其CAGradientLayer对象的frame，并插入view的layer
        gradientLayer.frame = CGRect(x: 0, y: 0, width: self.base.km.width, height: self.base.km.height)
    ...
    func gradientColorAnimation(direction: JKViewGradientDirection = .horizontal, startGradientColors: [Any], endGradientColors: [Any], duration: CFTimeInterval = 1.0, gradientLocations: [NSNumber]? = nil) {
        // 获取渐变对象
        let gradientLayer = CAGradientLayer().km.gradientLayer(direction, startGradientColors, gradientLocations)
        // 设置其CAGradientLayer对象的frame，并插入view的layer
        gradientLayer.frame = CGRect(x: 0, y: 0, width: self.base.km.width, height: self.base.km.height)
        self.base.layer.insertSublayer(gradientLayer, at: 0)
