...
public extension JKPOP where Base: UIImage {
    ...
    func addImageWatermark(rect: CGRect, image: UIImage) -> UIImage? {
        ...
        JKPrint(base.size)
        let s = base.size.width/SCREENHEIGHT
        image.draw(in: CGRect.init(x: base.size.width - rect.size.width*s, y:base.size.height - rect.size.height*s, width: rect.size.width*s, height: rect.size.height*s))
        let newImage = UIGraphicsGetImageFromCurrentImageContext()
        UIGraphicsEndImageContext()
        return newImage
    }
    ...
    static func textImage(_ text: String, fontSize: CGFloat = 16, size: (CGFloat, CGFloat), backgroundColor: UIColor = UIColor.orange, textColor: UIColor = UIColor.white, isCircle: Bool = true, isFirstChar: Bool = false) -> UIImage? {
        ...
        let textsize = text.km.rectSize(font: UIFont.systemFont(ofSize: fontSize), size: CGSize(width: kScreenW, height: CGFloat(MAXFLOAT)))
    ...
    func solidTo(size: CGSize) -> UIImage? {
        ...
        if self.base.size.height <= self.base.size.width {
            if self.base.size.width >= w {
                let scaleImage = fixOrientation().km.scaleTo(size: CGSize(width: w, height: w * self.base.size.height / self.base.size.width))
                return scaleImage
            } else {
            ...
            if self.base.size.height >= h {
                let scaleImage = fixOrientation().km.scaleTo(size: CGSize(width: h * self.base.size.width / self.base.size.height, height: h))
                return scaleImage
            } else {
                return fixOrientation()
            }
...
public extension JKPOP where Base: UIImage {
    ...
    func compress(mode: CompressionMode = .medium) -> Data? {
        return resizeIO(resizeSize: mode.resize(base.size))?.km.compressDataSize(maxSize: mode.maxDataSize)
    }
    ...
    func asyncCompress(mode: CompressionMode = .medium, queue: DispatchQueue = DispatchQueue.global(), complete:@escaping (Data?, CGSize) -> Void) {
        queue.async {
            let data = resizeIO(resizeSize: mode.resize(self.base.size))?.km.compressDataSize(maxSize: mode.maxDataSize)
            DispatchQueue.main.async {
...
public extension JKPOP where Base: UIImage {
    ...
    private static func getHDImgWithCIImage(with image: CIImage, size: CGSize, isLogo: Bool = true, logoSize: CGSize?, logoImage: UIImage? = nil, logoRoundCorner: CGFloat? = nil) -> UIImage? {
        ...
        if let newLogoRoundCorner = logoRoundCorner, let roundCornerLogo = logoImage.km.isRoundCorner(radius: newLogoRoundCorner, byRoundingCorners: .allCorners, imageSize: logoSize) {
