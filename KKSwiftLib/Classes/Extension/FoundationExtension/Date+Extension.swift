...
public extension JKPOP where Base == Date {
    ...
    static func timestampToFormatterDate(timestamp: String) -> Date {
        ...
        guard let timestampDouble = timestamp.km.toDouble() else {
            #if DEBUG
            fatalError("时间戳位有问题")
            #else
            return Date()
...
public extension JKPOP where Base == Date {
    ...
    var isYesterday: Bool {
        // 1.先拿到昨天的 date
        guard let date = Base().km.yesterDayDate else {
            return false
        }
        // 2.比较当前的日期和昨天的日期
        return Calendar.current.isDate(self.base, inSameDayAs: date)
    ...
    var isTheDayBeforeYesterday: Bool  {
        // 1.先拿到前天的 date
        guard let date = Base().km.theDayBeforYesterDayDate else {
            return false
        }
        // 2.比较当前的日期和昨天的日期
        return Calendar.current.isDate(self.base, inSameDayAs: date)
...
public extension JKPOP where Base == Date {
    ...
    static func currentMonthDays() -> Int {
        return daysCount(year: Date.km.currentDate.km.year, month: Date.km.currentDate.km.month)
    }
}
